...
import { applyCorrectionRule, getCorrectionKey } from './corrections.js';
...
async function isUrlAllowedForTracking(urlString = '') {
    const [allowlistEnabled, sites] = await Promise.all([
        getStorage(SITE_ALLOWLIST_ENABLED_KEY, false),
        getStorage(SITE_ALLOWLIST_KEY, [])
    ]);
    return isUrlAllowedByAllowlist(urlString, allowlistEnabled, sites);
}
async function getAllowlistStatus() {
...
async function addAllowlistSite(value) {
    ...
    const sites = await getStorage(SITE_ALLOWLIST_KEY, []);
    return setAllowlistSites([...sites, hostname]);
}
async function removeAllowlistSite(value) {
...
function mergeSessionSnapshot(base, incoming) {
    const next = ensureRecordIdentity(incoming);
    const output = { ...ensureRecordIdentity(base) };
    output.startedAt = Math.min(output.startedAt, next.startedAt);
    output.endedAt = Math.max(output.endedAt, next.endedAt);
...
async function updateLocalRecord(recordId, patch) {
    ...
    if ('manualTitle' in normalizedPatch || 'manualMediaType' in normalizedPatch || 'manualSeason' in normalizedPatch) {
        ...
        if (record && key) {
            const rules = await getStorage(CORRECTION_RULES_KEY, {});
...
async function getHistoryWithCloudFallback() {
    ...
    const localClearedAt = await getStorage(LOCAL_HISTORY_CLEARED_AT_KEY, null);
    if (localClearedAt) {
        return compacted;
    }
    try {
...
async function saveRecord(record) {
    ...
    const [history, correctionRules] = await Promise.all([
        ...
        getStorage(CORRECTION_RULES_KEY, {})
    ]);
...
async function saveActiveSessionSnapshot(tabId, now = Date.now()) {
...
async function startOrUpdateSession(tab, now = Date.now()) {
    ...
    const trackMutedVideos = await getStorage(TRACK_MUTED_KEY, false);
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    (async () => {
        ...
        if (payload?.type === 'getMutedTrackingStatus') {
            sendResponse({ ok: true, enabled: await getStorage(TRACK_MUTED_KEY, false) });
            return;
        }
