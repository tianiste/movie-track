...
async function openUrlWithResume(url: string, resumeAtSec: number): Promise<void> {
  ...
  const injectResume = async (): Promise<void> => {
...
function withResumeParam(urlString: string, resumeAtSec: number): string {
  if (resumeAtSec <= 0) {
    return urlString;
  }
...
function getStoredWatchUrl(urlString: string): string {
  const url = parseUrl(urlString);
  if (!url) {
    return urlString;
  }
...
async function getVideoPlaybackInfo(tabId: number): Promise<VideoPlaybackInfo | null> {
  try {
    ...
    const results = injections
      .map((injection) => injection.result as VideoPlaybackInfo | null)
      .filter((result): result is VideoPlaybackInfo => Boolean(result));
...
function getRecordIdentityCandidates(record: WatchRecord): string[] {
  const mediaType = record.mediaType ?? 'unknown';
  const season = record.season ?? 'x';
  const episode = record.episode ?? 'x';
...
function mergeIntoRecord(base: WatchRecord, incoming: WatchRecord): WatchRecord {
...
function compactHistory(history: WatchRecord[]): WatchRecord[] {
  ...
  for (const record of sorted) {
    ...
    const key = candidates.map((candidate) => candidateToKey.get(candidate)).find(Boolean) ?? (withIdentity.identityKey as string);
    const existing = byKey.get(key);
    if (!existing) {
    ...
    for (const candidate of getRecordIdentityCandidates(byKey.get(key) as WatchRecord)) {
...
chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
  (async () => {
    ...
    if (payload?.type === 'openWithResume') {
      ...
      const resumeAtSecRaw = (payload as { resumeAtSec?: unknown }).resumeAtSec;
